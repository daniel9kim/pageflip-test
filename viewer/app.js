...
let story='';
let spreads=[];
let current=0,locked=false,drag=null,tx=null;
const views={
  shelf:document.querySelector('#shelfView'),
...
function formatDate(date){
  if(!date) return '';
  const s=String(date).replaceAll('-','.');
  return s;
}
...
function photoSrc(p){
  ...
  const base=album.imageBase || 'webp/';
  return base.replace(/\/?$/,'/') + (p.file || p.f || '');
}
...
function normalizePhotos(list){
  return (list||[]).map((p,i)=>({
    ...
    orientation:photoOrientation(p)
  }));
}
...
async function loadAlbum(){
  ...
  const src=params.get('album') || 'album.json';
...
function bindAlbumToUI(){
  const title=album.title||'제목 없는 앨범';
  const date=formatDate(album.date);
  const year=yearOf(album.date);
  const summary=album.summary||'';
  const coverPhoto=photos[Math.max(0,Math.min(Number(album.coverIndex||0),photos.length-1))] || photos[0];
  const cover=coverPhoto ? photoSrc(coverPhoto) : '';

  document.title='PAGE FLIP · '+title;
  document.querySelector('#albumYear').textContent=year||'';
  document.querySelector('.shelf-section').dataset.year=year||'';
  document.querySelector('#shelfAlbumTitle').textContent=title;
  document.querySelector('#shelfAlbumDate').textContent=date;
  document.querySelector('#shelfCoverImg').src=cover;
  document.querySelector('#shelfCoverImg').alt=title+' 표지';

  document.querySelector('#coverMainImg').src=cover;
...
function makePage(item,side,no){
  ...
  if(item){
    ...
    i.alt='앨범 사진';
    d.appendChild(i);
  }else d.classList.add('blank-page');
...
function essaySpread(){
  ...
  const essayArt=album.essayImage
    ? `<img class="essay-art" src="${album.essayImage}" alt="에세이 이미지">`
    : (photos.length ? `<img class="essay-art" src="${photoSrc(photos[photos.length-1])}" alt="에세이 이미지">` : '');
  left.innerHTML=`<div class="essay-kicker">${escapeHtml(album.episodeLabel||'EPISODE 01')}</div>
    ...
    <div class="essay-quote">${album.storyQuote||'“사진 속에 남은 장면과 함께<br>그날의 생각도 한 장의 기록으로 남깁니다.”'}</div>
    ${essayArt}`;
  ...
  const short=excerpt.length>350?excerpt.slice(0,350).trim()+'…':excerpt;
...
function render(){
  ...
  if(s.type==='essay'){
    ...
    if(!mobile()) spreadEl.appendChild(r);
    else{
...
function setCurl(p,yRatio=.9){
  ...
  curl.style.width=cw+'px';curl.style.height=ch+'px';curl.style.right='0';curl.style.bottom='0';curl.style.opacity='1';
...
function animateTo(from,to,dur,done){
  ...
  function frame(t){
    const q=Math.min(1,(t-st)/dur),v=from+(to-from)*ease(q);
    setCurl(v,.88-v*.22);
...
function turn(delta){
  ...
  if(delta<0){current=next;render();return}
  locked=true;
  animateTo(.05,1,620,()=>{current=next;render();locked=false});
}
function openStory(){
...
document.querySelector('#storyModal').onclick=e=>{if(e.target.id==='storyModal')e.currentTarget.classList.remove('open')};
document.querySelector('#nextBtn').onclick=()=>turn(1);
document.querySelector('#prevBtn').onclick=()=>turn(-1);
addEventListener('keydown',e=>{
  if(document.querySelector('#storyModal').classList.contains('open')&&e.key==='Escape'){document.querySelector('#storyModal').classList.remove('open');return}
...
book.addEventListener('pointermove',e=>{
  ...
  if(drag){
    ...
    drag.p=p;drag.yRatio=(e.clientY-r.top)/r.height;setCurl(p,drag.yRatio);return;
  }
...
book.addEventListener('pointerdown',e=>{
  ...
  if(r.right-e.clientX<170&&r.bottom-e.clientY<170&&current<spreads.length-1){
    ...
    book.setPointerCapture(e.pointerId);book.classList.add('dragging');setCurl(.03,.9);
  }
});
book.addEventListener('pointerup',()=>{
  if(!drag)return;
  const p=drag.p||0;drag=null;
  if(p>.38){
    locked=true;animateTo(p,1,Math.max(260,560*(1-p)),()=>{current=Math.min(current+1,spreads.length-1);render();locked=false});
  }else{
    book.classList.add('settling');animateTo(p,0,360,resetCurl);
  }
});
...
document.querySelector('#search').addEventListener('input',e=>{
  ...
  document.querySelector('.bookcard').style.display=t.includes(e.target.value.trim().toLowerCase())?'':'none';
});
...
shareBtn.onclick=async()=>{
  ...
  try{
    if(navigator.share && location.protocol!=='file:'){await navigator.share({title,text,url});return}
    if(location.protocol==='file:'){toast('로컬 테스트입니다. 웹에 올리면 공유 링크를 사용할 수 있어요.');return}
    await navigator.clipboard.writeText(url);toast('앨범 링크를 복사했습니다.');
  }catch(err){
